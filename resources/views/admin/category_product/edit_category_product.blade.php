                    ...
                    <section class="panel">
                       ...
                       @foreach($edit_category_product as $key => $edit_value)
                            <div class="position-center">
                                ...
                                <div class="form-group">
                                    <label for="exampleInputPassword1">Depend on</label>
                                    <select name="depend_on" class="form-control input-sm m-bot15">
                                            <option value="0">----No depend----</option>
                                        @foreach($cate as $key => $cate)
                                            <option {{$edit_value->category_id == $cate->category_id ? ' selected="selected"' : ''}} value="{{$cate->category_id}}">{{$cate->category_name}}</option>
                                        @endforeach
